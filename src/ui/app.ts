...
const api = window.winCertbotApi;

async function loadSettings() {
  const res = await api.getSettings();
  if (res.ok && res.data) settingsState = res.data;
}
...
function render() {
  ...
  if (staging) staging.addEventListener('change', () => { settingsState.staging = staging.checked; settingsState.acmeDirectoryUrl = staging.checked ? 'https://acme-staging-v02.api.letsencrypt.org/directory' : 'https://acme-v02.api.letsencrypt.org/directory'; });
}
function pageContent(): string { if (current === 'dashboard') return `<div class="card"><h2>대시보드</h2><p>실시간 상태: ${statusText}</p><p>ACME 계정: ${accountKid}</p><button data-action="create-account">계정 생성/확인</button></div><div class="card"><h3>인증서 목록</h3><table><tr><th>도메인</th><th>상태</th><th>만료일</th></tr><tr><td>example.com</td><td>VALID</td><td>2026-08-10</td></tr></table></div>`; if (current === 'wizard') return wizard(); if (current === 'settings') return settings(); return `<div class="card"><h3>로그/진단</h3><p>재시도/진단 액션은 마법사 단계에서 실행 가능합니다.</p></div>`; }
function wizard(): string { const body = step===1?`<h3>Step1 도메인 입력</h3><input value="example.com"/>`: step===2?`<h3>Step2 Challenge 선택</h3><label><input type="radio"/> HTTP-01</label><label><input type="radio" checked/> DNS-01</label>`: step===3?`<h3>Step3A HTTP-01</h3><p>[ ] 파일 생성됨 [ ] 업로드 완료 [ ] 외부 접근 확인됨</p><button data-action="http-precheck">접근 확인(진단 실행)</button><button data-action="http-precheck">다시 시도</button>`: step===4?`<h3>Step3B DNS-01</h3><p>[ ] TXT 등록됨 [ ] 전파 확인됨 [ ] 검증 요청됨</p><button data-action="dns-check">전파 확인</button><button data-action="dns-check">재시도</button>`: `<h3>Step4 결과</h3><p>상태: ${statusText}</p><p>인증서 ID: cert-2026-0001</p><button data-page="dashboard">대시보드로 이동</button>`; return `<div class="card">${body}<div class="row"><button data-step="1">1</button><button data-step="2">2</button><button data-step="3">3(HTTP)</button><button data-step="4">3(DNS)</button><button data-step="5">4(결과)</button></div></div>`; }
function settings(): string { return `<div class="card"><h3>설정</h3><label><input id="staging-toggle" type="checkbox" ${settingsState.staging ? 'checked' : ''}/> Staging 사용</label><p>Directory: ${settingsState.acmeDirectoryUrl}</p><button data-action="save-settings">설정 저장</button></div>`; }
